...
from tensorflow.keras.layers import Activation, BatchNormalization, Conv2D, Conv3D, Dense, Flatten, MaxPooling3D, Input, Reshape, Dropout
...
def main():
    ...
    pixels2, labels, num_class = \
                    mydata.loadData(args.dataset, num_components=args.components, preprocessing=args.preprocess)
    shapeor = labels.shape
    pixels2, labels = mydata.createImageCubes(pixels2, labels, windowSize=args.spatialsize, removeZeroLabels = False)

    rstate = args.random_state+pos if args.random_state != None else None
    if args.dataset in ["UH", "DIP", "DUP", "DIPr", "DUPr"]:
        x_train, x_test, y_train, y_test = \
            mydata.load_split_data_fix(args.dataset, pixels2)#, rand_state=args.random_state+pos)
    else:
        pixels = pixels2[labels!=0]
    ...
    print("CNN3D_OV", args.dataset, args.tr_percent, args.spatialsize, args.idtest, results)
