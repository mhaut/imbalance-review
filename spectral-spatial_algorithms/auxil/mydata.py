...
def loadData(name, num_components=None, preprocessing="standard"):
	...
	num_class = 15 if name == "UH" else 9 if name in ["UP", "DUP", "DUPr"] else 13 if name == "KSC" else 16
...
def load_split_data_fix(name, pixels):
	...
	if name == "UH":
		...
		y_test = sio.loadmat(os.path.join(data_path, 'indian_pines_corrected_gt.mat'))['indian_pines_gt']
