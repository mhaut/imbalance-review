...
if __name__ == "__main__":
    ...
    labels = io.imread(osp.join(folder_dir, 'image_labels.tif'))
    print(labels.shape)
...
###############################################################################
    ...
    color_dict = {color: i for i, color in enumerate(get_labels())}

    alls=[]
    print(labels.shape)
    for sm in [labels[:,:1728,:], labels[:,1728:2240,:], labels[:,2240:,:]]:
        print("------")
        sm = sm.reshape(-1, labels.shape[-1])
        # for idl, l in enumerate(get_labels()):
        #
        #     sm2 = np.ones(sm.shape) * -100
        #     sm2[sm==l] = 1
        #     sm2 = color_dict[(255, 0, 0)]
        #     print(idl, np.unique(sm2, return_counts=1))
        # sm2 = np.ones((sm.shape[0], sm.shape[1])) * -100
        sm2 = []
        for a in sm:
            sm2.append(color_dict[tuple(a)])
        alls.append(np.unique(sm2, return_counts=1)[1])

    for idal,al in enumerate(alls):
        for idaa,aa in enumerate(al):
            print(idaa, aa)
        print()

    # print(np.unique(get_labels(labels[:,:1728,:]), return_counts=1))
    # print(np.unique(get_labels(labels[:,1728:2240,:]), return_counts=1))
    # print(np.unique(get_labels(labels[:,2240:,:]), return_counts=1))
# #
# #
# #
# #
