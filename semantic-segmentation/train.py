...
from helpers.lossfunctions import cross_entropy2d, focal_loss2d
...
from loss_functions import Cyclical_FocalLoss, ASL_FocalLoss
...
#        print(idx)
        ...
        try:
            loss = criterion(outputs, labels.to(device), epoch)
        except:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--network_weights_path', default = './modelosEntrenados/modelo.pt', help = 'Path to save Network weights')
    ...
    parser.add_argument('--lossF', default = 'cross-ent', help = 'Network architecture?')
    ...
    # weights = [0.06081359, 0.0175652, 0.02870433, 0.470368, 0.42254888, 0.0]
    weights = torch.FloatTensor(weights)

    if args.lossF   == "cross-ent":
        criterion = cross_entropy2d(reduction = 'mean', ignore_index = 5)
    elif args.lossF == "cross-ent-w":
        criterion = cross_entropy2d(reduction = 'mean', weight=weights.cuda(), ignore_index = 5)
    elif args.lossF == "focal":
        if args.network_arch == "unet":
            if args.args.mini:
                criterion = focal_loss2d(gamma=2.0, ignore_index=5, size_average=True)
        elif args.network_arch == "resnet":
            criterion = focal_loss2d(gamma=8.0, ignore_index=5)
    elif args.lossF == "aslfocal":
        if args.network_arch == "unet":
            if args.args.mini:
                criterion = ASL_FocalLoss(gamma=4, ignore_index=5)
        elif args.network_arch == "resnet":
            criterion = ASL_FocalLoss(gamma=10, ignore_index=5)
    elif args.lossF == "cfocal":
        criterion = Cyclical_FocalLoss(gamma_pos=4, gamma_neg=0, gamma_hc=2, epochs=60, factor=2)
