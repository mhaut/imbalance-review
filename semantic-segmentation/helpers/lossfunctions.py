...
class focal_loss2d(object):
    ...
    Parameters:
        ...
        weight          -- weight re-scaling given to each class, usually
                            calculated by median frequency scaling
    '''
    def __init__(self, reduction = 'mean', ignore_index = 999, weight = None, gamma=2.0):
        self.reduction = reduction
        self.ignore_index = ignore_index
        self.weight = weight
        self.floss = FocalLoss(gamma=gamma, reduction = reduction, weights=weight, ignore_index=ignore_index)
    ...
    def __call__(self, input, target):
        ...
        input = input.transpose(1, 2).transpose(2, 3).contiguous().view(-1, c)
        target = target.view(-1)
        loss = self.floss(input, target)
        # from focal_loss import FocalLoss
        # criterion = FocalLoss(gamma=2.0, reduction = 'mean', weights=weights.cuda(), ignore_index=5)
        return loss
